...
static NSTimeInterval const AWSS3TransferUtilityTimeoutIntervalForResource = 60 * 60 * 24 * 7; // 7 days
...
- (AWSTask<AWSS3TransferUtilityMultiPartUploadTask *> *)internalUploadFileUsingMultiPart:(NSURL *)fileURL
    ...
    return [[self.s3 createMultipartUpload:uploadRequest] continueWithBlock:^id(AWSTask *task) {
        ...
        for (int32_t i = 1; i <= partCount ; i++) {
            ...
            if (i == partCount) {
                dataLength = (NSUInteger)fileSize - ( (i-1) * AWSS3TransferUtilityMultiPartSize);
            }
                   ...
                   startTransfer: (BOOL) startTransfer
{
    ...
    if (!(subTask.file || [subTask.file isEqualToString:@""]) || ![[NSFileManager defaultManager] fileExistsAtPath:subTask.file]) {
        //Create a temporary file for this part.
        NSError *error = nil;
        NSString * partFileName = [self createTemporaryFileForPart:transferUtilityMultiPartUploadTask.file partNumber:[subTask.partNumber integerValue] dataLength:(NSUInteger)subTask.totalBytesExpectedToSend error:&error];
        if (partFileName == nil)  {
...
+ (void)interceptApplication:(AWSApplication *)application
